...
const Mostread = () => {
  ...
  return (
    <div  style={{ background: "white" }}>
      <h4 className='ms-3 mb-4 mt-4 border-bottom pb-2'>Най-четени</h4>
      <div className=' d-flex flex-wrap'>{recentPosts.length === 0
        ? <div className='p-3 text-muted'>Няма налични разгледани постове!</div>
        : recentPosts.map((p, i) => (
          <div key={p.id || i} className='d-flex  gap-2 mb-3 border-bottom pb-2'>
            <hr />
            ...
            <div style={{ fontSize: '14px', marginLeft: "2px", color: "#3a3a3a", fontWeight: 400, width: "10rem" }}> {p.Post.title}</div>
          </div>
        ))}</div>
    </div>
  )
